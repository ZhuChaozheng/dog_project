...
flag_database = list()  # for database thread
...
def weblink(c, addr):
    #  print('web', addr)
    sql = "SELECT * FROM dog_tbl WHERE time = (SELECT MAX(time) FROM dog_tbl)"
    # while 1:
        # try:
    threadLock.acquire()
    cursor.execute(sql)
...
def diff_web_sensors_data():
    ...
    while 1:
        if (cmp_dict(web_sensors_data, last_web_sensors_data) == 0):
            ...
            flag_database.append(web_sensors_data)
        last_web_sensors_data = web_sensors_data
        time.sleep(0.5)
    ...
    while True:
      ...
      VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % \
                  (web_sensors_data['equip_id'], web_sensors_data['yaw_one'], web_sensors_data['yaw_two'], web_sensors_data['pitch_one'],
                   ...
                   web_sensors_data['height'], dt, web_sensors_data['predictions'])
